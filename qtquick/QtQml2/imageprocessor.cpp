    ...
    bool event(QEvent *e)
    {
        if(e->type() == ExcutedEvent::evType()){
            ...
            if(m_runnables.contains(ee->m_runnable)){
                ...
                if(QFile(ee->m_runnable->m_destFilePath).exists())
...
static void _gray(QString sourceFile, QString destFile)
{
    QImage image(sourceFile.replace(0,8,""));
    if(image.isNull()){
        qDebug()<<"load"<<sourceFile.replace(0,7,"")<<"failed!";
        return;
    }
    ...
    for(int i=0; i<width; i++){
        for(int j=0; j<height; j++){
            if(image.valid(i, j) && image.valid(i+1, j) && image.valid(i, j+1)){
                ...
                g = qGreen(rgb00);
                b = qBlue(rgb00);
                gradientR = qAbs(r - qRed(rgb01)) + qAbs(r - qRed(rgb10));
                gradientG = qAbs(g - qGreen(rgb01)) + qAbs(r - qGreen(rgb10));
                gradientB = qAbs(b - qBlue(rgb01)) + qAbs(r - qBlue(rgb10));
                if(gradientR > threshold){
                    r = qMin(gradientR + 100, 255);
                }
                if(gradientG > threshold){
                    r = qMin(gradientG + 100, 255);
                }
                if(gradientB > threshold){
                    r = qMin(gradientB + 100, 255);
                }
